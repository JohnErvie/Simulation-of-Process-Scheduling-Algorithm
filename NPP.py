...
class NPPWin(QMainWindow):
    ...
    def Labels(self):
        titleLabel = QLabel("None Pre-emptive Priority", self)
        titleLabel.setGeometry(QRect(30+150+80,50, 900, 100))
    ...
    def Table(self):
        self.row = 1
        self.column = 4
        self.NPPTable = QTableWidget(self.row,self.column,self)
        self.NPPTable.setGeometry(QRect(100,50+100, 975, 650))
        
        self.NPPTable.setHorizontalHeaderLabels(("Process ID", "Arrival Time", "Burst Time", "Priority"))
        self.NPPTable.setColumnWidth(0,238)
        self.NPPTable.setColumnWidth(1,238)
        self.NPPTable.setColumnWidth(2,238)
        self.NPPTable.setColumnWidth(3,238)
    ...
    def clickedAdd(self):
        ...
        self.NPPTable.insertRow(self.rowCount)
    ...
    def clickedDelete(self):
        if self.NPPTable.rowCount() > 0:
            self.NPPTable.removeRow(self.NPPTable.rowCount()-1)
        ...
        if self.NPPTable.rowCount() == 1:
            self.deleteButton.hide()

        self.updateDelRow()

    def updateAddRow(self):
        self.rowCount = self.NPPTable.rowCount()
        self.onlyInt = QIntValidator()
        for i in range(1,4): 
            self.tableLE = QLineEdit()
            self.tableLE.setFont(QtGui.QFont('Times New Roman',14))
            self.tableLE.setValidator(self.onlyInt)
            self.NPPTable.setCellWidget(self.NPPTable.rowCount()-1, i, self.tableLE)
        
        if self.NPPTable.rowCount() > 0:
            for x in range(0,self.NPPTable.rowCount()):
                self.tableLable = QLineEdit()
                self.tableLable.setFont(QtGui.QFont('Times New Roman',14))
                self.NPPTable.setCellWidget(self.NPPTable.rowCount()-1, 0, self.tableLable)
    ...
    def updateDelRow(self):
        ...
        for i in range(1,3): 
            ...
            self.tableLE.setFont(QtGui.QFont('Times New Roman',14))
            self.tableLE.setValidator(self.onlyInt)
            self.NPPTable.setCellWidget(self.NPPTable.rowCount(), i, self.tableLE)
        
        if self.NPPTable.rowCount() > 0:
            for x in range(0,self.NPPTable.rowCount()):
                self.tableLable = QLineEdit()
                self.tableLable.setFont(QtGui.QFont('Times New Roman',14))
                self.NPPTable.setCellWidget(self.NPPTable.rowCount(), 0, self.tableLable)
    ...
    def _clickedCal(self):
        ...
        for row in range(0,self.rowCount):
            ...
            for col in range(0,4):
                item = self.NPPTable.cellWidget(row, col)
                item_text = item.text()
                if col_index > 0:
                    if item_text == '':
        ...
        # check if there is/are the same process ID
            ...
            global NPP_values
            NPP_values = self.valTables
            self._NPP_ResultWin = NPP_ResultWin()
            self._NPP_ResultWin.show()
            self.hide()
...
class NPP_ResultWin(QMainWindow):
    ...
    def variables(self):
        ...
        lengthNPP_valTables = len(self.NPP_valTables)

        NPP = 4

        allProcess = int(lengthNPP_valTables/NPP)
        ...
        for row in range(allProcess): # Converting the values to 2d array
            for col in range(NPP):
                listedVal[row].append(self.NPP_valTables[indexVal])
                indexVal += 1
        ...
        while loop != False: 
            ...
            if savedBurstTime < 1:
                    ...
                    if int(len(queue)) > 1 : # if more than 1 in queue check the lowest priority
                        ...
                        if int(len(samePriority)) > 1:
                            ...
                            if int(len(sameBurstTime)) > 1:
                                ...
                                while loopqueue != False:
                                    ...
                                    while rowat < int(len(sameBurstTime)):
                                        if int(queue[int(sameBurstTime[rowat])][1]) == lowat:
                                            ...
                                            savedBurstTime = int(queue[int(sameBurstTime[rowat])][2])
                                            savedRow = int(sameBurstTime[rowat])
                                            rowat = int(len(queue))
                                            loopqueue = False
                                        rowat +=1
                                    lowat += 1
                                ...
                                savedBurstTime = int(queue[int(sameBurstTime[0])][2])
                                savedRow = int(sameBurstTime[0])
                        else:
                            queue[int(samePriority[0])][2] = int(queue[int(samePriority[0])][2]) - 1
                            savedBurstTime = int(queue[int(samePriority[0])][2])
                            savedRow = int(samePriority[0])
                        
                    else: # if only 1 process in queue then execute it
                        queue[0][2] = int(queue[0][2]) - 1 # subtract 1 burst time
                        savedRow = 0
                        savedBurstTime = int(queue[0][2])
                ...
                queue[savedRow][2] = int(queue[savedRow][2]) - 1
                savedBurstTime -= 1
    ...
    def resultButtons(self):
        backButton = QPushButton('Back to NPP', self)
        backButton.setGeometry(QRect(150,850, 150, 50))
        #backButton.setStyleSheet("QWidget {background-color: Blue}")
        backButton.setFont(QtGui.QFont('Times New Roman',14))
        backButton.clicked.connect(self.clickedBack)
