...
PP_values = ['p1', 3, 4, 2, 0, 0, 0, 'p2', 5, 9, 1, 0, 0, 0, 'p3', 8, 4, 2, 0, 0, 0, 'p4', 0, 7, 1, 0, 0, 0, 'p5', 12, 6, 1, 0, 0, 0]
...
class PP_ResultWin(QMainWindow):
    def __init__(self):
        ...
        self.title = "Pre-emptive Priority Result"
        self.width = 1200
        self.height = 950

        global PP_values
        self.PP_valTables = PP_values
        self.lengthPP_valTables = len(self.PP_valTables)

        self.allProcess = int(self.lengthPP_valTables/7)
        ...
        for row in range(self.allProcess): # Converting the values to 2d array
            for col in range(7):
                self.listedVal[row].append(self.PP_valTables[self.indexVal])
                self.indexVal += 1
        ...
        while i < self.lengthPP_valTables:
            self.processID.append(self.PP_valTables[i])
            i += 7
    ...
    def resultLabels(self):
        titleResultLabel = QLabel("Pre-emptive Priority", self)
        titleResultLabel.setGeometry(QRect(30+150+30+130,10, 900, 100))
            ...
            if int(len(self.queue)) > 0:
                if int(len(self.queue)) > 1 : # if more than 1 in queue check the lowest priority
                    ...
                    if int(len(self.samePriority)) > 1:
                        ...
                        else: # if only 1 process in burst time queue then execute it
                            self.queue[int(self.sameBurstTime[0])][2] = int(self.queue[int(self.sameBurstTime[0])][2]) - 1 # subtract 1 burst time
                    ...
                    else: # if only 1 process in priority queue then execute it
                        self.queue[int(self.samePriority[0])][2] = int(self.queue[int(self.samePriority[0])][2]) - 1 # subtract 1 burst time
