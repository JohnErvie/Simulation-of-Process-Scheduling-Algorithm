...
class SJFWin(QMainWindow):
    def __init__(self):
        super().__init__()

        self.title = "Shortest Job First"
        self.width = 1200
        self.height = 950
    ...
    def Labels(self):
        titleLabel = QLabel("Shortest Job First", self)
        titleLabel.setGeometry(QRect(30+150+125+55,50, 900, 100))
    ...
    def Table(self):
        self.row = 1
        self.column = 3
        self.SJFTable = QTableWidget(self.row,self.column,self)
        self.SJFTable.setGeometry(QRect(100,50+100, 975, 650))
        
        self.SJFTable.setHorizontalHeaderLabels(("Process ID", "Arrival Time", "Burst Time"))
        self.SJFTable.setColumnWidth(0,316)
        self.SJFTable.setColumnWidth(1,316)
        self.SJFTable.setColumnWidth(2,316)
    ...
    def clickedAdd(self):
        self.SJFTable.insertRow(self.rowCount)
        ...
        self.rowCount = self.SJFTable.rowCount()
        self.onlyInt = QIntValidator()
        
        self.updateAddRow()

    def clickedDelete(self):
        if self.SJFTable.rowCount() > 0:
            self.SJFTable.removeRow(self.SJFTable.rowCount()-1)
    ...
    def updateAddRow(self):
        ...
        for i in range(1,3): 
            ...
            self.SJFTable.setCellWidget(self.SJFTable.rowCount()-1, i, self.tableLE)
        
        if self.SJFTable.rowCount() > 0:
            for x in range(0,self.SJFTable.rowCount()):
                self.tableLable = QLineEdit()
                self.tableLable.setFont(QtGui.QFont('Times New Roman',14))
                self.SJFTable.setCellWidget(self.SJFTable.rowCount()-1, 0, self.tableLable)
    ...
    def updateDelRow(self):
        ...
        for i in range(1,3): 
            ...
            self.tableLE.setFont(QtGui.QFont('Times New Roman',14))
            self.tableLE.setValidator(self.onlyInt)
            self.SJFTable.setCellWidget(self.SJFTable.rowCount(), i, self.tableLE)
        
        if self.SJFTable.rowCount() > 0:
            for x in range(0,self.SJFTable.rowCount()):
                self.tableLable = QLineEdit()
                self.tableLable.setFont(QtGui.QFont('Times New Roman',14))
                self.SJFTable.setCellWidget(self.SJFTable.rowCount(), 0, self.tableLable)
    ...
    def _clickedCal(self):
        ...
        for row in range(0,self.rowCount):
            ...
            for col in range(0,3):
                item = self.SJFTable.cellWidget(row, col)
                item_text = item.text()
                if col_index > 0:
                    if item_text == '':
        ...
        # check if there is/are the same process ID
            ...
            global SJF_values
            SJF_values = self.valTables
            self._SJF_ResultWin = SJF_ResultWin()
            self._SJF_ResultWin.show()
            self.hide()
